...
- `internal/models/models.go` — domain structs (User, Project, Response, Role, Notification)
- `internal/repo/` — database layer, one file per entity, raw SQL queries
...
- `static/js/app.js` — notifications dropdown, user menu, CSRF helper
...
**Database:** SQLite with WAL mode, foreign keys enabled. Connection opened in `repo.New()` which also runs goose migrations. JSON arrays (skills, stack) stored as TEXT columns.
...
- Form arrays: `r.Form["roles"]` for checkbox groups
